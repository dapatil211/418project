...
int main(){
    ...
    // ifstream fin("enwik8.comp", ios::in);
    ifstream fin("test.comp", ios::in);
    vector<char> input;
    vector<char> output;
    readInput(fin, input);
    fin.close();
    Model m = Model(5, 255);
    decompress(input, output, m, 0, input.size());
    cout << "Finished decompressing " << input.size() << endl;
    ofstream fout("test.dec", ios::out);
